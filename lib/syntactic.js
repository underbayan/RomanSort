...
exports.syntactic = (tokens) => {
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (item.scope & scopeType.comment || !item.scopeText.trim()) {
            ...
            blockList.push({ mergedText, firstLine: getFirstLineTextTrim(item.scopeText, mergedText) })
            mergedText = ""
        }
    }
    if (mergedText !== '') {
        blockList.push({ mergedText, firstLine: getFirstLineTextTrim(mergedText, mergedText) })
    }
    return blockList
}
